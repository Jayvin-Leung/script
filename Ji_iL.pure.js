var BiliService = (function() {
	...
	function getOption(isOpenNote) {
		...
		if (videoInfo.type == 1) {
			return page_combinator['single_page']();
		}

		if (videoInfo.type == 2) {
			return page_combinator['multi_page']();
		}
		...
		if (videoInfo.type == 3) {
			...
			if (ioc) {
				return { ...{isOnlyCurr: true}, ...page_combinator['single_page']() };
			} else {
				return { ...{isOnlyCurr: false}, ...page_combinator['multi_page']() };
			}
		}
	...
	class Video {
		...
		download() {
			createAndDownloadFile(this.files, this.exNote.images());
		}
	...
	function createAndDownloadFile(notes, images) {
...
一、使用说明
	...
	3. 修改 BiliService(null, null) 中的两个 null 分别为 option 和 BV，只修改其中一个，另一个保持 null
...
二、修改示例
	...
	3. 括号中填入 option 和 BV
		a. BiliService(null, null);     默认配置 + 当前视频
		b. BiliService(option, null);   自定义配置 + 当前视频
		c. BiliService(null, BV);       默认配置 + 指定视频
		d. BiliService(option, BV);     自定义配置 + 指定视频
...
var bili = new BiliService(null, null);
bili.toNote();
bili = null;